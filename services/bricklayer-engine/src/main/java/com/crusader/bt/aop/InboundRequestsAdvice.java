...
public class InboundRequestsAdvice {
    ...
    public Object aroundRestControllersAdvice(ProceedingJoinPoint joinPoint) throws Throwable {
        ...
        log.info("Catch inbound request to engine service: {}", methodSignature);

        Object response = joinPoint.proceed();
        if (response instanceof Mono<?> out) {
            return out
                    .doOnSuccess(resp ->
                            log.info("Send outbound response from engine service")
                    )
                    .doOnError(e ->
                            log.error(
                                    "Catch Exception while process inbound request into engine service: {} - {}",
                                    methodSignature,
                                    e
                            )
                    );
