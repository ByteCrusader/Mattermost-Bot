...
public class LoggingClientFilter {
    ...
    public ExchangeFilterFunction logRequestFunction() {
        return ExchangeFilterFunction.ofRequestProcessor(clientRequest -> {
            log.info("Request from bff service to {} has been sent", clientRequest.url().getPath());
    ...
    public ExchangeFilterFunction logResponseHttpStatusFunction() {
        return ExchangeFilterFunction.ofResponseProcessor(clientResponse -> {
            log.info("Received response into bff service with status {}", clientResponse.statusCode());
