...
public class KafkaConfiguration {
    ...
    @Bean
    public KafkaReceiver<String, MessageDto> bffMessageReceiver() {
        return KafkaReceiver.create(
                ReceiverOptions
                        ...
                        .subscription(Collections.singleton(kafkaProperties.getBffQueue().getTopic()))
        );
    }
    ...
    private Map<String, Object> buildConsumerFactoryConfig(KafkaProperties details) {
        ...
        props.put(ConsumerConfig.GROUP_ID_CONFIG, details.getBffQueue().getClientNGroupId());
        props.put(ConsumerConfig.CLIENT_ID_CONFIG, details.getBffQueue().getClientNGroupId());
